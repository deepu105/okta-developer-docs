...
This guide explains embedded authentication and the fundamentals of the Auth.js SDK.
...
The Okta Auth SDK builds on top of our APIs to enable you to create a fully branded and customized sign-in experience using JavaScript.
...
> **Note:** `@okta/okta-auth-js` version x.y.z or above is required to run samples in this guide.

If you'd like to explore the entire Auth SDK, see the [Okta Auth JavaScript SDK][authjs-reference].

## Installation

Include the following script tag in your target web page with the latest version:

```html
<script src="https://global.oktacdn.com/okta-auth-js/${SDK-version}/okta-auth-js.min.js" type="text/javascript"></script>
```

[Or install sdk with NPM? -- **Brian]
...
``` javascript
const authClient = new OktaSignIn({
...
Replace each of these property values with those from your Okta org and app integration. For more information about these properties, see the [Client Configuration section of the Auth SDK reference][authjs-reference-client-configuration].

## Pass in sign-in credentials and authenticate

Pass in the username and password from the customized sign-in form for your application. Use the `idx.authenticate` method to authenticate the user. See [idx.authenticate](https://github.com/okta/okta-auth-js/blob/master/docs/idx.md#idxauthenticate).
...
A successful authentication returns an `Idx.Status` of `SUCCESS` from Okta.
...
With a successful authentication status, `IdxStatus.SUCCESS`, save the token from the response, and processes the authenticated user in the app.
...
[This is the classic content below -- for reference -- it will be removed]
